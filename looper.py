...
# Add the following code after the while loop

# Create a Tkinter window
root = tk.Tk()
root.title("Audio Looper")

# Create sliders for each track
volume_sliders = []
for i in range(4):
    slider = Scale(root, from_=0, to=100, orient="horizontal", label=f"Track {i+1} Volume")
    slider.pack()
    volume_sliders.append(slider)

# Function to update volume based on sliders
def update_volume():
    for i, slider in enumerate(volume_sliders):
        loops[i].output_volume = slider.get() / 100.0
    updatevolume()  # Update the overall output volume

# Create a button to apply volume changes
apply_button = tk.Button(root, text="Apply Volume", command=update_volume)
apply_button.pack()
